...
public class ItemSpawner : MonoBehaviour {
    ...
    public float timeBetSpawnMax = 7f; // 아이템 생성 최대 시간 간격
    ...
    private void Spawn() {
        ...
        // 바닥에 아이템이 딱 붙어서 생성되지 않도록 바닥에서 0.5만큼 위로 올리기
        spawnPosition += Vector3.up * 0.5f;

        // 아이템 중 하나를 무작위로 골라 랜덤 위치에 생성
        GameObject selectedItem = items[Random.Range(0, items.Length)];
        GameObject item = Instantiate(selectedItem, spawnPosition, Quaternion.identity); //오일러각(0,0,0)에대응
    ...
    private Vector3 GetRandomPointOnNavMesh(Vector3 center, float distance) {
        ...
        // 내비메시 위의 모든 영역(NavMesh.AllAreas)에 대해 maxDistance 반경 안에서, randomPos에 가장 가까운 내비메시 위의 한 점을 찾는 샘플링을 실행하여 결과를 hit에 저장
